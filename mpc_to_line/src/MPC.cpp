...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
	...
	// Minimize cte , psi and dif in velocity
	for (int t = 0; t < N; t++) {
	...
	//minimoze heavy use of actuators
	for (int t = 0; t < N - 1; t++) {
		fg[0] += CppAD::pow(vars[delta_start + t],2);
		fg[0] += CppAD::pow(vars[a_start + t],2);
	}
	
	//minimoze dif between sequential actuations
	for (int t = 0; t < N - 2; t++) {
    ...
    // It is Vehicle Model equations at each point should follow Kinemetic 
	// model equations
    ...
    for (int t = 1; t < N; t++) { // starting at 1 , taking diff between 
...
int main() {
  ...
  std::cout<<"x_vals : ";
  for(size_t s = 0 ; s<x_vals.size() ; s++){
	std::cout<<x_vals[s]<<",";
  }
  std::cout<<std::endl;

  std::cout<<"y_vals : ";
  for(size_t s = 0 ; s<y_vals.size() ; s++){
	std::cout<<y_vals[s]<<",";
  }
  std::cout<<std::endl;
 
  std::cout<<"cte_vals : ";
  for(size_t s = 0 ; s<cte_vals.size() ; s++){
	std::cout<<cte_vals[s]<<",";
  }
  std::cout<<std::endl;
  
   std::cout<<"delta_vals : ";
  for(size_t s = 0 ; s<delta_vals.size() ; s++){
	std::cout<<delta_vals[s]<<",";
  }
  std::cout<<std::endl;
  
  std::cout<<"v_vals : ";
  for(size_t s = 0 ; s<v_vals.size() ; s++){
	std::cout<<v_vals[s]<<",";
  }
  std::cout<<std::endl;
